...
import { FourtwoCognitoIdentityStackExportsZod } from "./exports";
...
export = async () => {
	...
	return all([identityPoolOutput]).apply(([idpool]) => {
		...
		} satisfies z.infer<typeof FourtwoCognitoIdentityStackExportsZod>;

		const validate = FourtwoCognitoIdentityStackExportsZod.safeParse(exported);
		if (!validate.success) {
