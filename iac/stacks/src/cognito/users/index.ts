...
import { FourtwoCognitoUsersStackExportsZod } from "./exports";
...
export = async () => {
	...
	const pools = (() => {
		...
		return {
			chat: userpool("chat", {}),
			moderation: userpool("moderation", {
	...
	return all([userpoolsOutput]).apply(([userpools]) => {
		const exported = {
			fourtwo_cognito_users_userpool: userpools,
		} satisfies z.infer<typeof FourtwoCognitoUsersStackExportsZod>;

		const validate = FourtwoCognitoUsersStackExportsZod.safeParse(exported);
		if (!validate.success) {
