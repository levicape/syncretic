import type { PipelineOptionsBuilder } from "../cd/pipeline/PipelineOptionsBuilder.mjs";
import { getCommitMessage, isMainBranch } from "../machine/code/Git.mjs";

type PipelineOptionsBuilderUnknown = PipelineOptionsBuilder<{ id: string }>;

const isDocumentationFile = (filename: string) =>
	/^(\.vscode|\.github|bench|docs|examples)|\.(md)$/i.test(filename);
const isTestFile = (filename: string) =>
	/^test/i.test(filename) || /runner\.node\.mjs$/i.test(filename);

export class CommitMessages {
	static force = ({
		changedFiles,
	}: Pick<PipelineOptionsBuilderUnknown, "changedFiles">) => {
		...
		for (const coref of [
			...
			if (changedFiles?.includes(coref)) {
				console.log(
					" - Yes, because the list of changed files contains:",
					coref,
				);
				forceBuild = true;
				ciFileChanged = true;
	...
	static skipCi = ({
		...
		if (!forceBuild) {
			...
			if (
				changedFiles?.every((filename: string) => isDocumentationFile(filename))
			) {
	...
	static buildImages = ({
		ciFileChanged,
	}: Pick<PipelineOptionsBuilderUnknown, "ciFileChanged">) => {
	...
	static publishImages = ({
		...
		if (ciFileChanged && isMainBranch()) {
			console.log(
				" - Yes, because a core CI file changed and this is main branch",
			);
			publishImages = true;
			buildImages = true;
		}
	...
	static skipBuild = ({
		...
		if (!forceBuild) {
			const message = getCommitMessage();
			const match =
				/\[(only tests?|tests? only|skip build|no build|build skip|build no)\]/i.exec(
					message ?? "",
				);
			if (match) {
				const [, reason] = match;
			...
			if (
				changedFiles?.every(
					(filename: string) =>
						isTestFile(filename) || isDocumentationFile(filename),
				)
			) {
				console.log(
					" - Yes, because all changed files are tests or documentation",
				);
				skipBuild = true;
			}
		}
	...
	static skipTests = () => {
		...
		const message = getCommitMessage();
		const match = /\[(skip tests?|tests? skip|no tests?|tests? no)\]/i.exec(
			message ?? "",
		);
		if (match) {
	...
	static release = () => {
		...
		const message = getCommitMessage();
		const match = /\[(release|release build|build release)\]/i.exec(
			message ?? "",
		);
		if (match) {
			const [, reason] = match;
