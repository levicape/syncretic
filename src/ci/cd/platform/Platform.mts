...
export type PlatformPrototype<Step> = {
	...
	getEphemeralAgent: (
		version: "v1" | "v2",
		instance: { image?: string; instanceType: string },
	) => Agent;
	getTestAgent: () => Agent;
	getBuildAgent: () => Agent;
...
export class Platform {
	...
	static getTestAgent = (
		...
		const { arch, os, release, isUsingNewAgent, getEphemeralAgent } =
			new PlatformBuilder()
				.setOs(platform.os)
		...
		return getEphemeralAgent("v1", {
			instanceType: undefined as unknown as string,
		});
	};
