...
export const AwsCodebuildGithubOidcProjectParameter = (principal?: string) =>
	`/fourtwo/${principal ? `${principal}` : "_principal"}/codebuild/github/oidc/CodebuildProjectArn`;
...
export const AwsCodebuildGithubOidcCommand = async () => {
	return async () =>
		buildCommand({
			loader: async () => {
				return async (flags: Flags) => {
					...
					const oaaParameter = AwsOrganizationPrincipalOAAParameter(principal);
					const oaaRole = (
					...
					);
					{
						...
						const oidcParameter =
							AwsOrganizationPrincipalOIDCParameter(principal);
						const oidcRole = (
							await root.GetParameter({
								Name: oidcParameter,
							})
						...
						let previousId = (
							await systems.GetParameter({
								Name: AwsCodebuildGithubOidcProjectParameter(),
							})
						)?.Parameter?.Value as
						...
						let parameter = AwsCodebuildGithubOidcProjectParameter();
						let scopedParameter = AwsCodebuildGithubOidcProjectParameter(
							await AwsPrincipalNameFromPackageJson(),
						);
